...
**Свойства с использованием `gen/bind` (зависимые генераторы):**
- `remove-existing-key` - удаление гарантированно существующего ключа
- `update-existing-key` - обновление существующего ключа перезаписывает значение
- `ifn-lookup-existing` - вызов словаря как функции работает на существующем ключе

Каждое свойство проверяется на 100 случайно сгенерированных тестовых случаях. Итого: **1700 проверок** (17 свойств × 100 итераций).
...
Я познакомилась с property-based тестированием через библиотеку test.check. Вместо того чтобы писать конкретные примеры, я описала свойства, которые должны выполняться для любых данных (например, "после вставки элемент можно найти"). Библиотека сама генерирует 100 случайных тестов для каждого свойства и проверяет их. Получилось 17 свойств × 100 итераций = 1700 проверок. Реализованы зависимые генераторы через `gen/bind`, которые позволяют тестировать операции на гарантированно существующих элементах.
